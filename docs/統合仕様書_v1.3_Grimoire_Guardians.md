...
changelog:
  ...
  - version: "1.3"
    date: "2026-02-19"
    changes: "Week3仕様追加: ResultScreen詳細(星評価/花丸演出/ClearScreen統合)、EventManager追加、streak状態追加、IndexedDB fallback方針"
  - version: "1.4"
    date: "2026-02-22"
    changes: "デプロイ戦略追加(8-4): GitHub Pages（開発用）→ 本番移行の手順・タイミングを明記"
  - version: "1.2"
    date: "2026-02-15"
...
const saveDataSchema = {
  ...
  core: {
    ...
    currentSession: {
      ...
      shieldActive: false,
      streak: 0,        // ← v1.3追加: 現在の連続正解数（セッション開始時リセット）
      maxStreak: 0      // ← v1.3追加: セッション中の最大連続正解数（ResultScreenで表示用）
    },
...
#### 結果画面（v1.3詳細化・ClearScreen統合）
- **目的**: スコア表示、クリア判定、達成演出
- **クリア条件**: 60%以上正解
- **表示モード**: `mode: 'clear' | 'perfect' | 'fail'` で内部切り替え
  - ※旧「クリア演出画面（ClearScreen）」をこの画面に統合
...
- **星評価**:
  ...
  - ⭐⭐⭐  90〜100%
- **演出**:
  1. 星がひとつずつ飛び込んでくる（bounce-in）
  2. スター数カウントアップアニメーション
  3. 報酬倍率（おみくじバフ）適用演出（倍率あり時のみ）
  4. 獲得素材カードめくり演出（1枚ずつ順に表示）
  5. セーブデータ更新（cleared: true、次ワールド解放）
- **ボタン**: 「もう一いかい」「本棚へ」

##### 🌸 未クリア時（60%未満）— がんばったで賞
- **花丸演出**（SVG stroke-dashoffset によるペン書き風）:
  1. `0.0s` 画面に暖かいピンクのオーバーレイ
  2. `0.1s` 大きな花丸がゆっくり描かれる（0.8秒、太い手書き線）
  3. `0.9s` 円が閉じた瞬間に ✨ 8方向バースト
  4. `1.0s` 「はなまる！」スタンプ（中央、-15deg、scale bounce）
  5. `1.2s` 🌸🌟💫 シャワー（JS生成20個、2秒間ふわふわ落下）
  6. `1.8s` 「よく がんばったね！」スライドイン
  7. `2.0s` 「まえより ○もん ふえたよ！」（記録比較、初回は非表示）
  8. `2.5s` ボタン出現
- **ボタン**: 「もう一いかい！」「本棚へ」
...
class SaveManager {
  ...
  // IndexedDB 利用不可時のフォールバック（v1.3追加）
  // プライベートブラウジング・Safari ITP等で IndexedDB が使えない場合:
  //   1. localStorage にフォールバック（透過的 — 呼び出し元は変更不要）
  //   2. 画面上部にバナー表示「データが一時保存中です」
  //   3. ゲームの動作は継続（データ消失リスクをユーザーに通知するのみ）
...
│   │   ├── LicenseManager.js
│   │   ├── DropSystem.js
│   │   ├── EventManager.js       ← v1.3追加（イベント発火・管理の統括）
│   │   ├── RewardMultiplier.js
│   │   └── ErrorBoundary.js
│   │
│   ├── components/         # 再利用可能UI
...
#### 開発中（Phase 0〜Phase 1）：GitHub Pages を使う

**GitHub Pages とは？（初めての方向け）**

GitHubが無料で提供している「ウェブサイト公開サービス」です。
GitHubにコードを保存（push）するだけで、誰でもブラウザからアクセスできる
URLが自動的に作られます。特別なサーバー契約や難しいサーバー設定は不要です。

```
公開URL（例）: https://ikawakens-create.github.io/grimoire-guardians/
```

**GitHub Pages の設定方法（最初に1回だけ行う操作）:**

```
1. GitHubのリポジトリページを開く
2. 上部の「Settings（設定）」タブをクリック
3. 左メニューの「Pages」をクリック
4. 「Source（ソース）」を「main」ブランチ、フォルダは「/ (root)」に設定
5. 「Save（保存）」をクリック
   → 数分後にURLが発行される
```

**開発中に GitHub Pages を使うメリット:**

| メリット | 説明 |
|----------|------|
| 無料 | 費用ゼロで使える |
| 自動更新 | `main` ブランチにコードを反映するだけで自動更新 |
| 実機確認に最適 | スマホ・タブレットでの動作確認ができる |
| 共有が簡単 | URLを送るだけでテストプレイヤーに見せられる |

---

#### ⚠️ 製品リリース前に必ず行うこと（GitHub Pages からの移行）

**なぜ GitHub Pages のままでは製品版として出せないのか？**

| 問題 | 詳細 |
|------|------|
| 商業利用の制限 | GitHubの利用規約により、収益化したサービスへの継続利用は原則不可 |
| URLがブランドとして弱い | `github.io` 固定で、独自ドメイン（例: `grimoire-guardians.jp`）が使いにくい |
| サーバー側の処理ができない | 課金・ライセンス認証・ランキングなどの機能は GitHub Pages では動かせない |

**移行手順（製品リリース時に行う操作）:**

```
Step 1: GitHub Pages を無効化する
  - GitHub Settings > Pages > Source を「None（なし）」に変更
  - 理由: 商業利用制限を回避するため

Step 2: 本番ホスティングサービスに移行する（候補）
  ┌─────────────┬──────────────────────────────┬─────────────────┐
  │ サービス    │ 特徴                         │ おすすめ時期    │
  ├─────────────┼──────────────────────────────┼─────────────────┤
  │ Netlify     │ 無料プランあり・PWA対応・    │ Phase 1 ベータ  │
  │             │ 独自ドメイン可               │ テスト時        │
  ├─────────────┼──────────────────────────────┼─────────────────┤
  │ Vercel      │ 高速CDN・無料プランあり      │ Phase 1 ベータ  │
  │             │                              │ テスト時        │
  ├─────────────┼──────────────────────────────┼─────────────────┤
  │ 独自サーバー│ 完全な自由度                 │ 製品リリース時  │
  │             │ （課金・ライセンス処理も可） │                 │
  └─────────────┴──────────────────────────────┴─────────────────┘

Step 3: 独自ドメインを取得する（製品リリース時）
  - 例: grimoire-guardians.jp
  - 費用の目安: 年間 1,000〜3,000 円程度
  - 取得場所: お名前.com、さくらドメイン など

Step 4: アプリストア展開（将来）
  - Android: TWA（Trusted Web Activity）でPlayストア申請
  - iOS: WKWebView ラッパーでAppStore申請
```

**移行タイミングの目安:**

```
Phase 0（現在）  ─→ GitHub Pages（開発・テスト用）  ← 今ここ
Phase 1          ─→ Netlify or Vercel に移行（ベータテスト開始）
製品リリース     ─→ 独自ドメイン + 本番サーバー（課金・ライセンス対応）
アプリ展開時     ─→ TWA（Android）/ WKWebView（iOS）でアプリストア申請
```

> **まとめ:** GitHub Pages は「作りながら試す」ための一時的な置き場所です。
> 製品として販売・配信する段階になったら、必ず本番環境に移行してください。
...
- v1.3: Week3仕様追加（2026-02-19）
  - ResultScreen詳細化（星評価⭐〜⭐⭐⭐、花丸演出、ClearScreen統合）
  - がんばったで賞（未クリア時の花丸演出・全7フェーズ）
  - EventManager.js をファイル構成に追加
  - currentSession に streak / maxStreak を追加
  - SaveManager の IndexedDB fallback 方針を明記
...
**バージョン:** 1.4
**ステータス:** 承認済み・凍結
**次のステップ:** Phase 0.1 Week3 実装開始

**v1.3での主要追加:**
- ResultScreen の完全仕様化（星評価・花丸演出・ClearScreen統合）
- がんばったで賞（SVG花丸アニメーション全7フェーズ）
- EventManager によるイベント管理の一元化
- currentSession.streak でストリーク機能を実装可能に
- SaveManager の IndexedDB 利用不可時の安全な fallback 設計

**v1.4での主要追加:**
- デプロイ戦略（8-4）: GitHub Pages（開発用）→ 本番環境への移行手順・タイミングを明記
- 製品版リリース前に GitHub Pages を無効化する旨を明記
