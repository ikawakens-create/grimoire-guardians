...
> **注意**：プロジェクトルート直下に `mon_*.png.png` などの画像残骸が散乱しています。技術的負債として後日 `assets/monsters/` へ整理予定です。
...
> **[技術的負債]**: `GameStore` の初期ステートが巨大化しています（house・ship・town・guild等）。将来的にドメインごとのモジュール分割が推奨されます。
...
> **[技術的負債]**: `index.js` の `hideAll()` が全画面を個別にハードコーディングしており、スケールしづらい。将来的に `ScreenManager` への移行を推奨。
...
| 優先度 | 課題 | 対応方針 |
|-------|------|---------|
| 高 | **Router の汎用化** | `index.js` の `hideAll()` を `ScreenManager`（配列/Map管理）にリファクタリング |
| 高 | **GameStore 分割** | 巨大化した state を `TownState`・`HouseState`・`ShipState` 等にドメイン分割 |
| 中 | **アセットディレクトリ整理** | ルート直下の `mon_*.png.png` を `assets/monsters/` に移動・参照パスを修正 |
